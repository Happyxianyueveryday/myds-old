  ...
  由于C++没有内置的无穷大表示，因此如下的代码统一在在邻接矩阵中使用-1表示两个结点不直接相连（即距离为无穷大），如果使用python语言来实现，由于python中有无穷大表示float('inf')，因此实现代码将更为简单和简洁，本处的代码仅供参考。
	...
	for(int i=0;i<size-1;i++)   // 上述循环只需进行size-1次结束，因为集合N中的结点数目为size-1 
	{
		...
		{ 
			if(visit[u]==false)
			{
				...
				// 特别地，C++的INT_MAX不是严格的无穷大表示，不能参与数值运算，否则会导致溢出为负数，在python中使用float('inf')表示无穷大时，参与运算时也不会导致溢出
...
{	
  ...
  需要补充的是: 在kruskkal算法中，需要判断一个图是否有环，判断一个图是否存在环的算法非常朴素，对该图进行深度优先遍历(dfs)或者广度优先遍历(bfs)，只要在遍历过程中遇到了之前已经访问过的结点，则该图中必然存在环。具体的算法实现请参考如下的代码。
