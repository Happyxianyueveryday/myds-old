...
bool DiGraph::topoSort1(vector<int> &res)
{
	...
	for(int i=0;i<size;i++)    // 将原图中的全部入度为0的结点入栈
	{
		int flag=0;
		for(int k=0;k<size;k++)
		{
			if(_data[k][i]!=DBL_MAX)
			{
				flag=1;
				break;
			}
	...
	while(sta.size())
	{
		...
		res.push_back(now);   // 将当前结果放入结果中

		for(int i=0;i<size;i++)
		{
			_data[now][i]=DBL_MAX;   // 删除当前结点到其他后继结点的边

			int flag=0;
			for(int k=0;k<size;k++)   // 判断该后继结点在删除后是否入度为0，若后继结点入度为0则将其放入结果中
			{
				if(_data[k][i]!=DBL_MAX)
				{
					flag=1;
