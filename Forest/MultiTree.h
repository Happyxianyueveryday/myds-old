...
class MultiTree
{
    ...
    MultiTree(TreeNode *root);    							   // 构造函数，从给定的多叉树根结点指针构造多叉树 

    // 2. copy controllers
    MultiTree(const MultiTree &other);                // 拷贝构造函数
    MultiTree & operator = (const MultiTree &other);  // 拷贝赋值运算符
...
/**
 * MultiTree: 从给定的多叉树根结点指针构造多叉树 
 * note: 注意必须从给定的指针中深拷贝一份多叉树，而不能使用浅拷贝，否则容易出现错误 
*/ 
MultiTree::MultiTree(TreeNode *root)
{
	treeroot=__copyTree(root);
}
...
TreeNode *MultiTree::__copyTree(TreeNode *root)
{
	if(!root)
	return NULL;
	else
	{
		...
		res->child=__copyTree(root->child);
		res->brother=__copyTree(root->brother);
