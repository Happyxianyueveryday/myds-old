...
void Sort::insert_sort(int lis[], int size)
{
	for(int i=0;i<size;i++)
	{
		...
		while(k>=0&&lis[k]>tar)    // 从后向前查找直到第一个lis[k]<=tar的元素，则lis[k+1]就是插入点，从后往前找的目的是为了减少开销，从前向后也可以，但是开销较大 
		{
			lis[k+1]=lis[k];
			k--;
		}
		
		lis[k+1]=tar;              // 数组的插入移动需要仔细思考，这里一开始写成了lis[k]=tar，出现了错误，因为lis[k]<tar，因此插入点应该是lis[k+1] 
	}
