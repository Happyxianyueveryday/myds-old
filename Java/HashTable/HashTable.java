...
class HashTable
{
	...
	public boolean insert(int key)
	{
		...
		else 
		{
			...
			else if(this.method=="square")
			{
				...
				{
					int pos=(index+i*i)%this.max_size;  
	 ...
	 * @note: 哈希表的查找过程和插入过程在基本上类似，但是也有所不同，注意同时检查当前探查的位置是否被使用（即要求dic[pos]==true），且当前探查的位置的值是否等于所查询的值（即要求data[pos]==key）
	 */
	public int getindex(int key)
	{
		...
		if(this.dic[index]==true&&this.data[index]==key)
			return index;
